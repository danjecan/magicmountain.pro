...
def nav(lang, active_path=None):
    ...
    links_mobile = "".join(a(k, h) for k, h in NAV_ITEMS)
    return f'''<header>
<div class="nav">
  ...
  <div class="nav-links">
    ...
    <button type="button" class="nav-menu-btn" id="navMenuBtn" aria-expanded="false" aria-controls="navMobile" aria-label="{t("menu_open", lang)}">
      {icon("menu", "ico", "width:26px;height:26px;stroke-width:2;")}
    </button>
  </div>
</div>
<nav class="nav-mobile" id="navMobile" aria-label="{t("nav_about", lang)}">
  {links_mobile}
  <a href="{signup_href}" class="btn btn-primary">{t("nav_cta", lang)}</a>
</nav>
</header>
'''
...
def hero(img, eyebrow, title_html, lead, ctas="", tags="", home=False, alt=""):
  ...
  <img src="{img}" alt="{esc(alt)}"><div class="hero-shade"></div>
